...
DEFAULT_ENV_PATH: Path = Path('/etc/grandstart/grandstart.env')
...
class AppSettings(BaseSettings):
    ...
    config_path: str = '/etc/grandstart/config.yml'
    debug: bool = False
...
def load_settings(env_file_path: str | None = None, env_file_encoding: str | None = None,
    ...
    if env_file_path is None:
        env_file_path = os.getenv('GRST_ENV_FILE', DEFAULT_ENV_PATH)
    ...
    if env_file_encoding is None:
        env_file_encoding = os.getenv('GRST_ENV_FILE_ENCODING', 'UTF-8')

    # Extract the default secrets directory path from the environment if defined, otherwise use the default path
    if secrets_path is None:
        secrets_path = os.getenv('GRST_ENV_SECRETS_DIR', DEFAULT_SECRETS_PATH)
    ...
    os.putenv('GRST_ENV_FILE', str(env_file_path))
    os.putenv('GRST_ENV_FILE_ENCODING', env_file_encoding)
    ...
    if secrets_path is not None:
        ...
        if valid:
            ...
            os.putenv('GRST_ENV_SECRETS_DIR', str(secrets_path))
