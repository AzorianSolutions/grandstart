...
CONTEXT_SETTINGS = dict(auto_envvar_prefix="GRST")
...
def cli(ctx: Environment):
    ...
    if ctx.settings.debug:
        logger.remove()
        logger.add(sys.stderr, colorize=True, format="<green>{time}</green> <level>{level}</level> <cyan>{module}.{"
                                                     "name}:{function}:{line}</cyan> <level>{message}</level>",
                   level="TRACE")
